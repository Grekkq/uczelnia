...
//Początek i koniec listy książek
struct ksiazka {
	ksiazka_ele *head;
	ksiazka_ele *tail;
};

//Lista wskaźników na książki
struct wsk_ksiazka_ele {
	ksiazka_ele * ptr_to_ksiazka;
	wsk_ksiazka_ele * next;
};
//Początek i koniec listy wskaźników na książki
struct wsk_ksiazka {
...
//Początek i koniec listy etykiet
struct lista {
	lista_ele *head;
	lista_ele *tail;
};
...
int main() {
	...
	while (getline(plik, rec)) {
		...
		do {
				//wymyślić jak przeczytać tylko jedną etykiete i ja wyizolować i nie zapomnieć reszty bo później do niej przejść
